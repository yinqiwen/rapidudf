...
// Per-target
#if defined(HIGHWAY_HWY_CONTRIB_SORT_VQSORT_TOGGLE) == defined(HWY_TARGET_TOGGLE)
...
template <class Traits, typename T, typename V>
void SiftDown(Traits st, T* HWY_RESTRICT lanes, V* HWY_RESTRICT vals, const size_t num_lanes, size_t start) {
  ...
  while (start < num_lanes) {
    ...
    if (right < num_lanes && AllTrue(d, st.Compare(d, st.SetKey(d, lanes + idx_larger), st.SetKey(d, lanes + right)))) {
      idx_larger = right;
    }
...
template <class Traits, typename T, typename V>
void HeapSort(Traits st, T* HWY_RESTRICT lanes, V* HWY_RESTRICT vals, const size_t num_lanes) {
  constexpr size_t N1 = st.LanesPerKey();
...
template <class Traits, typename T, typename V>
void HeapSelect(Traits st, T* HWY_RESTRICT lanes, V* HWY_RESTRICT vals, const size_t num_lanes, const size_t select) {
  constexpr size_t N1 = st.LanesPerKey();
...
template <class Traits, typename T, typename V>
void HeapPartialSort(Traits st, T* HWY_RESTRICT lanes, V* HWY_RESTRICT vals, const size_t num_lanes,
                     const size_t select) {
...
HWY_INLINE void Sort2To2(Traits st, T* HWY_RESTRICT keys, size_t num_lanes, T* HWY_RESTRICT /* buf */) {
...
template <class Traits, typename T>
HWY_INLINE void Sort3To4(Traits st, T* HWY_RESTRICT keys, size_t num_lanes, T* HWY_RESTRICT buf) {
  constexpr size_t kLPK = st.LanesPerKey();
  const size_t num_keys = num_lanes / kLPK;
...
HWY_INLINE void CopyHalfToPaddedBuf(D d, Traits st, T* HWY_RESTRICT keys, size_t num_lanes, T* HWY_RESTRICT buf) {
...
HWY_NOINLINE void Sort8Rows(Traits st, T* HWY_RESTRICT keys, size_t num_lanes, T* HWY_RESTRICT buf) {
...
template <size_t kKeysPerRow, class Traits, typename T>
HWY_NOINLINE void Sort16Rows(Traits st, T* HWY_RESTRICT keys, size_t num_lanes, T* HWY_RESTRICT buf) {
  ...
  Merge16x2<kKeysPerRow>(d, st, v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, va, vb, vc, vd, ve, vf);
  Merge16x4<kKeysPerRow>(d, st, v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, va, vb, vc, vd, ve, vf);
  Merge16x8<kKeysPerRow>(d, st, v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, va, vb, vc, vd, ve, vf);
#if !HWY_COMPILER_MSVC && !HWY_IS_DEBUG_BUILD
  Merge16x16<kKeysPerRow>(d, st, v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, va, vb, vc, vd, ve, vf);
#endif
...
template <class D, class TraitsKV, typename T, typename V>
HWY_NOINLINE void BaseCase(D d, TraitsKV, T* HWY_RESTRICT keys, V* HWY_RESTRICT vals, size_t num_lanes, T* buf,
                           V* val_buf) {
  ...
  const size_t ceil_log2 = 32 - Num0BitsAboveMS1Bit_Nonzero32(static_cast<uint32_t>(num_keys - 1));
...
HWY_INLINE size_t PartitionRightmost(D d, Traits st, T* const keys, const size_t num, const Vec<D> pivot, size_t& bufL,
...
HWY_INLINE void StoreLeftRight(D d, Traits st, const Vec<D> v, const Vec<D> pivot, T* HWY_RESTRICT keys, size_t& writeL,
                               size_t& remaining) {
  const size_t N = Lanes(d);
  ...
  remaining -= N;
  if (hwy::HWY_NAMESPACE::CompressIsPartition<T>::value || (HWY_MAX_BYTES == 16 && st.Is128())) {
...
template <class D, class Traits, typename T>
HWY_INLINE void StoreLeftRight4(D d, Traits st, const Vec<D> v0, const Vec<D> v1, const Vec<D> v2, const Vec<D> v3,
                                const Vec<D> pivot, T* HWY_RESTRICT keys, size_t& writeL, size_t& remaining) {
...
template <class D, class Traits, typename T>
HWY_INLINE void StoreRightAndBuf(D d, Traits st, const Vec<D> v, const Vec<D> pivot, T* HWY_RESTRICT keys,
                                 size_t& writeR, T* HWY_RESTRICT buf, size_t& bufL) {
  const size_t N = Lanes(d);
  const Mask<D> comp = st.Compare(d, pivot, v);
  const size_t numL = CompressStore(v, Not(comp), d, buf + bufL);
  bufL += numL;
...
template <class D, class Traits, typename T>
HWY_INLINE size_t Partition(D d, Traits st, T* const keys, const size_t num, const Vec<D> pivot, T* HWY_RESTRICT buf) {
  using V = decltype(Zero(d));
  const size_t N = Lanes(d);

  size_t bufL, writeR;
  const size_t num_main = PartitionRightmost(d, st, keys, num, pivot, bufL, writeR, buf);
  HWY_DASSERT(num_main <= num && writeR <= num);
  HWY_DASSERT(bufL <= Constants::PartitionBufNum(N));
  HWY_DASSERT(num_main + bufL == writeR);

  if (VQSORT_PRINT >= 3) {
    fprintf(stderr, "  num_main %zu bufL %zu writeR %zu\n", num_main, bufL, writeR);
  }
  ...
  if (HWY_LIKELY(num_main != 0)) {
    ...
    while (readL != readR) {
      ...
      const size_t capacityL = static_cast<size_t>((readL - keys) - static_cast<ptrdiff_t>(writeL));
...
template <class D, class Traits, typename T>
HWY_NOINLINE bool MaybePartitionTwoValue(D d, Traits st, T* HWY_RESTRICT keys, size_t num, const Vec<D> valueL,
                                         const Vec<D> valueR, Vec<D>& third, T* HWY_RESTRICT buf) {
  const size_t N = Lanes(d);
...
template <class D, class Traits, typename T>
HWY_INLINE bool MaybePartitionTwoValueR(D d, Traits st, T* HWY_RESTRICT keys, size_t num, const Vec<D> valueL,
                                        const Vec<D> valueR, Vec<D>& third, T* HWY_RESTRICT buf) {
  const size_t N = Lanes(d);
  ...
  if (VQSORT_PRINT >= 2) {
    fprintf(stderr, "MaybePartitionTwoValueR countR %zu pos %zu i %zu endL %zu\n", countR, pos, i, endL);
  }
...
template <class D, class Traits, typename T>
HWY_INLINE bool PartitionIfTwoKeys(D d, Traits st, const Vec<D> pivot, T* HWY_RESTRICT keys, size_t num,
                                   const size_t idx_second, const Vec<D> second, Vec<D>& third, T* HWY_RESTRICT buf) {
  // True if second comes before pivot.
  const bool is_pivotR = AllFalse(d, st.Compare(d, pivot, second));
  if (VQSORT_PRINT >= 1) {
    fprintf(stderr, "Samples all equal, diff at %zu, isPivotR %d\n", idx_second, is_pivotR);
  }
  ...
  return is_pivotR ? MaybePartitionTwoValueR(d, st, keys, num, second, pivot, third, buf)
                   : MaybePartitionTwoValue(d, st, keys + idx_second, num - idx_second, pivot, second, third, buf);
}

// Second path into `MaybePartitionTwoValue`, called when not all samples are
// equal. `samples` is sorted.
template <class D, class Traits, typename T>
HWY_INLINE bool PartitionIfTwoSamples(D d, Traits st, T* HWY_RESTRICT keys, size_t num, T* HWY_RESTRICT samples) {
...
template <class D, class Traits, typename T>
HWY_INLINE void DrawSamples(D d, Traits st, T* HWY_RESTRICT keys, size_t num, T* HWY_RESTRICT buf,
  ...
  const size_t misalign = (reinterpret_cast<uintptr_t>(keys) / sizeof(T)) & (kLanesPerChunk - 1);
  if (misalign != 0) {
  ...
  const uint32_t num_chunks = static_cast<uint32_t>(HWY_MIN(num_chunks64, 0xFFFFFFFFull));
...
HWY_INLINE bool UnsortedSampleEqual(D d, Traits st, const TFromD<D>* HWY_RESTRICT samples) {
...
template <class D, class Traits, typename T>
HWY_INLINE Vec<D> ChoosePivotByRank(D d, Traits st, const T* HWY_RESTRICT samples) {
...
template <class D, class Traits, typename T>
HWY_INLINE bool AllEqual(D d, Traits st, const Vec<D> pivot, const T* HWY_RESTRICT keys, size_t num,
  ...
  // Vector-align keys + i.
  const size_t misalign = (reinterpret_cast<uintptr_t>(keys) / sizeof(T)) & (N - 1);
  ...
  size_t i = consume;
  HWY_DASSERT(((reinterpret_cast<uintptr_t>(keys + i) / sizeof(T)) & (N - 1)) == 0);
...
template <class D, class Traits, typename T>
HWY_INLINE bool ExistsAnyBefore(D d, Traits st, const T* HWY_RESTRICT keys, size_t num, const Vec<D> pivot) {
  const size_t N = Lanes(d);
...
// Called from 'two locations', but only one is active (IsKV is constexpr).
template <class D, class Traits, typename T>
HWY_INLINE bool ExistsAnyAfter(D d, Traits st, const T* HWY_RESTRICT keys, size_t num, const Vec<D> pivot) {
  const size_t N = Lanes(d);
  HWY_DASSERT(num >= N);  // See HandleSpecialCases
...
// (thus the right partition will never be empty).
template <class D, class Traits, typename T>
HWY_INLINE Vec<D> ChoosePivotForEqualSamples(D d, Traits st, T* HWY_RESTRICT keys, size_t num, T* HWY_RESTRICT samples,
                                             Vec<D> second, Vec<D> third, PivotResult& result) {
...
template <class D, class Traits, typename T>
HWY_NOINLINE void PrintMinMax(D d, Traits st, const T* HWY_RESTRICT keys, size_t num, T* HWY_RESTRICT buf) {
  if (VQSORT_PRINT >= 2) {
    const size_t N = Lanes(d);
    if (num < N) return;
...
HWY_NOINLINE void Recurse(D d, Traits st, T* HWY_RESTRICT keys, const size_t num, T* HWY_RESTRICT buf,
                          uint64_t* HWY_RESTRICT state, const size_t remaining_levels, const size_t k = 0) {
  HWY_DASSERT(num != 0);
  ...
  if (HWY_UNLIKELY(UnsortedSampleEqual(d, st, buf))) {
    ...
    if (HWY_UNLIKELY(!st.IsKV() && PartitionIfTwoKeys(d, st, pivot, keys, num, idx_second, second, third, buf))) {
    ...
    pivot = ChoosePivotForEqualSamples(d, st, keys, num, buf, second, third, result);
    ...
    // Not supported for key-value types because two 'keys' may be equivalent
    // but not interchangeable (their values may differ).
    if (HWY_UNLIKELY(!st.IsKV() && PartitionIfTwoSamples(d, st, keys, num, buf))) {
      return;
    }
  ...
  HWY_IF_CONSTEXPR(mode == RecurseMode::kSelect) {
    if (HWY_LIKELY(result != PivotResult::kIsFirst) && k < bound) {
      ...
      Recurse<RecurseMode::kSelect>(d, st, keys + bound, num - bound, buf, state, remaining_levels - 1, k - bound);
    }
  }
  HWY_IF_CONSTEXPR(mode == RecurseMode::kSort) {
    ...
    if (HWY_LIKELY(result != PivotResult::kWasLast)) {
      Recurse<RecurseMode::kSort>(d, st, keys + bound, num - bound, buf, state, remaining_levels - 1);
    }
  }
}

// Returns true if sorting is finished.
template <class D, class Traits, typename T, typename V>
HWY_INLINE bool HandleSpecialCases(D d, Traits st, T* HWY_RESTRICT keys, V* HWY_RESTRICT vals, size_t num,
                                   T* HWY_RESTRICT buf, V* HWY_RESTRICT val_buf) {
  const size_t N = Lanes(d);
  constexpr size_t kLPK = st.LanesPerKey();
  ...
  if (HWY_UNLIKELY(num <= base_case_num)) {
    ...
    BaseCase(d, st, keys, vals, num, buf, val_buf);
    return true;
  }
  ...
  constexpr bool kPotentiallyHuge = HWY_MAX_BYTES / sizeof(T) > Constants::kMaxRows * Constants::kMaxCols;
...
void Sort(D d, Traits st, T* HWY_RESTRICT keys, const size_t num, T* HWY_RESTRICT buf) {
  if (VQSORT_PRINT >= 1) {
    fprintf(stderr, "=============== Sort num %zu is128 %d isKV %d vec bytes %d\n", num, st.Is128(), st.IsKV(),
...
template <class D, class Traits, typename T, typename V>
void PartialSort(D d, Traits st, T* HWY_RESTRICT keys, V* HWY_RESTRICT vals, size_t num, size_t k, T* HWY_RESTRICT buf,
                 V* HWY_RESTRICT val_buf) {
  if (VQSORT_PRINT >= 1) {
...
#if VQSORT_ENABLED || HWY_IDE
  if (!detail::HandleSpecialCases(d, st, keys, vals, num, buf, val_buf)) {  // TODO
    uint64_t* HWY_RESTRICT state = hwy::detail::GetGeneratorStateStatic();
    // Introspection: switch to worst-case N*logN heapsort after this many.
    // Should never be reached, so computing log2 exactly does not help.
    const size_t max_levels = 50;
    ...
    detail::Recurse<detail::RecurseMode::kSelect>(d, st, keys, num, buf, state, max_levels, k);
    detail::Recurse<detail::RecurseMode::kSort>(d, st, keys, k, buf, state, max_levels);
  }
#else   // !VQSORT_ENABLED
  (void)d;
  (void)buf;
...
template <class D, class Traits, typename T, typename V>
HWY_API void PartialSort(D d, Traits st, T* HWY_RESTRICT keys, V* HWY_RESTRICT vals, const size_t num, const size_t k) {
  HWY_ASSERT(k < num);
  constexpr size_t kLPK = st.LanesPerKey();
  constexpr size_t buf_len = SortConstants::BufBytes<T, kLPK>(HWY_MAX_BYTES) / sizeof(T);
  HWY_ALIGN T buf[buf_len];
  HWY_ALIGN V val_buf[buf_len];
  PartialSort(d, st, keys, vals, num, k, buf, val_buf);
}
...
void VQSortStatic(K* HWY_RESTRICT keys, V* HWY_RESTRICT vals, const size_t num, SortDescending) {
#if VQSORT_ENABLED
  using Adapter = detail::KeyAdapter<K>;
...
template <typename K, typename V>
void VQPartialSortStatic(K* HWY_RESTRICT keys, V* HWY_RESTRICT vals, const size_t num, const size_t k, SortAscending) {
#if VQSORT_ENABLED
  using Adapter = detail::KeyAdapter<K>;
  using Order = typename Adapter::Ascending;
  const detail::SharedTraits<typename Adapter::template Traits<Order>> st;
  using LaneType = typename decltype(st)::LaneType;
  const SortTag<LaneType> d;
  PartialSort(d, st, reinterpret_cast<LaneType*>(keys), vals, num * st.LanesPerKey(), k);
#else
  (void)keys;
  (void)num;
  (void)k;
...
template <typename K, typename V>
void VQPartialSortStatic(K* HWY_RESTRICT keys, V* HWY_RESTRICT vals, const size_t num, const size_t k, SortDescending) {
#if VQSORT_ENABLED
  using Adapter = detail::KeyAdapter<K>;
  using Order = typename Adapter::Descending;
  const detail::SharedTraits<typename Adapter::template Traits<Order>> st;
  using LaneType = typename decltype(st)::LaneType;
  const SortTag<LaneType> d;
  PartialSort(d, st, reinterpret_cast<LaneType*>(keys), num * st.LanesPerKey(), k);
#else
  (void)keys;
  (void)num;
  (void)k;
...
template <typename K, typename V>
void VQSelectStatic(K* HWY_RESTRICT keys, V* HWY_RESTRICT vals, const size_t num, const size_t k, SortAscending) {
#if VQSORT_ENABLED
  using Adapter = detail::KeyAdapter<K>;
  using Order = typename Adapter::Ascending;
  const detail::SharedTraits<typename Adapter::template Traits<Order>> st;
...
template <typename K, typename V>
void VQSelectStatic(K* HWY_RESTRICT keys, V* HWY_RESTRICT vals, const size_t num, const size_t k, SortDescending) {
#if VQSORT_ENABLED
  using Adapter = detail::KeyAdapter<K>;
  using Order = typename Adapter::Descending;
  const detail::SharedTraits<typename Adapter::template Traits<Order>> st;
